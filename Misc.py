...
def get_indices(net, constraints):
    ...
    return list(net.blobs.keys()),list(indices[::-1])
...
def show_progress(x, net, title=None, handle=False):
    ...
    print('Progress:',time.localtime()[3:6])
...
def load_image(file_name, im_size, net_model, net_weights, mean, show_img=False):
    ...
    with open(net_model,'r+') as f:
        f.writelines(data)
#    net_mean = np.tile(mean[:],(1,)+tuple(im_size.astype(int)))
    net_mean =  np.tile(mean[:,np.newaxis,np.newaxis],(1,) + tuple(im_size.astype(int)))
