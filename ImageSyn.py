...
from Misc import *
def ImageSyn(net, constraints1, constraints2, alpha,  init=None, bounds=None, callback=None, minimize_options=None, gradient_free_region=None):
    '''
    ...
    '''
    #print('type of initial image:', type(init))
    #iprint('shape of initial image:', init.shape)
    if init ==None:
        init = np.random.randn(*net.blobs['data'].data.shape)
    
     #get indices for gradient
    layers, indices = get_indices(net, constraints1)
#     layers2, indices2 = get_indices(net, constraints2)
    ...
    def f(x):
        ...
        for i,index in enumerate(indices):
            layer = layers[index]
            for l,loss_function in enumerate(constraints1[layer].loss_functions):
                constraints1[layer].parameter_lists[l].update({'activations': net.blobs[layer].data.copy()})
                val, grad = loss_function(**constraints1[layer].parameter_lists[l])
                f_val += alpha * val
                net.blobs[layer].diff[:] += alpha * grad
            for l,loss_function in enumerate(constraints2[layer].loss_functions):
                constraints2[layer].parameter_lists[l].update({'activations': net.blobs[layer].data.copy()})
                val, grad = loss_function(**constraints2[layer].parameter_lists[l])
                f_val += (1 - alpha) * val
                net.blobs[layer].diff[:] += (1 - alpha) * grad
