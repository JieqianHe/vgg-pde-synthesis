...
def gram_mse_loss(activations, target_gram_matrix, diag, weight=1., linear_transform=None):
    '''
    ...
    if linear_transform == None:
        ...
        if not diag:
            ...
            loss = float(weight)/4 * ((np.diag(np.diag(G - G_target)))**2).sum() / N
            gradient = (weight * np.dot(F.T, np.diag(np.diag(G - G_target))).T / (M * N)).reshape(1, N, fm_size[0], fm_size[1])
    else: 
        F = np.dot(linear_transform, activations.reshape(N,-1))
        G = np.dot(F,F.T) / M
        if diag:
            loss = float(weight)/4 * ((np.diag(np.diag(G - G_target)))**2).sum() / N
            gradient = (weight * np.dot(linear_transform.T, np.dot(F.T, np.diag(np.diag(G - G_target))).T) / (M * N)).reshape(1, N, fm_size[0], fm_size[1])
        else:
            loss = float(weight)/4 * ((G - G_target)**2).sum() / N**2
